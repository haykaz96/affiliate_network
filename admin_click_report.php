...
if (isset($_POST['Submit']) && $_POST['Submit']=='Delete') {
    if (count($_POST["selected_id"]) > 0) {
        ...
        foreach ($idArr as $id) {
            $query3 = "delete from uc_clicks where id='$id'";
            $run = mysqli_query($mysqli, $query3);
        }
        if ($run > 0) {
            if (!empty($dt)) {

                echo '<script>window.location="admin_click_report.php?dt=' . $dt . '&status=delete_click";</script>';
            } else {
                echo '<script>window.location="admin_click_report.php?dt=' . $dt . '&status=delete_click";</script>';
            }
        }
...
if (isset($_POST['btn_change'])) {
    if (count($_POST["selected_id"]) > 0) {
        ...
        foreach ($idArr as $id) {
            ...
            $delivered = "UPDATE uc_clicks SET status='$status_arr' WHERE  id='$id'";
        ...
        if ($run == 1) {
            if (!empty($dt)) {
                ...
                echo '<script>window.location="admin_click_report.php?dt=' . $dt . '&status=change_click_status";</script>';
            } else {
                echo '<script>window.location="admin_click_report.php?status=change_click_status";</script>';
            }
        }
    }
}
...
<?php if ($_GET['status'] == 'delete_click') { $successes[] = lang("ADMIN_DELETE_CLICK"); echo resultBlock($errors, $successes); } ?>
<?php if ($_GET['status'] == 'change_click_status') { $successes[] = lang("ADMIN_CLICK_STATUS_CHANGED"); echo resultBlock($errors, $successes); } ?>
...
<td><a href="admin_click_report.php?user_id=<?php echo $row['user_id']; ?><?php echo $link_url; ?>"><?php echo $row['user_id']; ?></a></td>
<td><?php $date = date_create($row['date_time']); echo date_format($date, "H:i:s d/m/Y"); ?></td>
<td><a href="admin_click_report.php?offer_id=<?php echo $row['offer_id']; ?><?php echo $link_url; ?>"><?php echo $row['offer_id']; ?></a></td>
<td><?php echo $row['sub_id']; ?></td>
<td><a href="admin_click_report.php?ip_address=<?php echo $row['ip_address']; ?><?php echo $link_url; ?>"><?php echo $row['ip_address']; ?></a></td>
<td><a href="admin_click_report.php?status=<?php echo $row['status']; ?><?php echo $link_url; ?>"><?php echo getClickStatusColour($row['status']); ?></a></td>
...
<script>
    jQuery(function () {
        jQuery('#row').on('change', function () {
            ...
            if (url) {
                window.location = 'admin_click_report.php?row=' + this.value + "<?php echo $temp_link; ?>"
            }
            return false;
        });
    });
...
<script>
    jQuery(function () {
        jQuery('#time_range').on('change', function () {
            var url = jQuery(this).val();
            if (url) {
                window.location = 'admin_click_report.php?dt=' + this.value
            }
            return false;
        });
    });
