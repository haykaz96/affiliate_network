...
if (($_GET['dt'] == "") && ($_GET['row'] == "")) {
    $targetpage = "publisher-conversion-report.php?";
} elseif (!empty($_GET['dt']) && !empty($_GET['row'])) {
    $targetpage = "publisher-conversion-report.php?row=" . $_GET['row'] . "&dt=" . $_GET['dt'] . "&";
} else {
    $targetpage = "publisher-conversion-report.php?row=" . $_GET['row'] . "&dt=" . $_GET['dt'] . "&";
}
...
<h2>Publisher Conversion Report</h2>
<p>Use the form below to view all conversions sent to our offers.</p>
...
<td><a href="campaigns.php?offer=<?php echo $row['offer_id']; ?>">
<?php 
...
<td><?php $status = $row['status']; echo getConversionStatusColour($status); ?></td>

<?php } ?>
</tr>
<?php } else { ?>
<tr>
<td colspan="7">No Conversions Found.</td>
</tr>
<?php } ?>
</table>
...
<script>
    jQuery(function () {
        jQuery('#time_range').on('change', function () {
            ...
            if (url) {
                window.location = 'publisher-conversion-report.php?dt=' + this.value
            }
            return false;
        });
    });
...
<script>
    jQuery(function () {
        jQuery('#row').on('change', function () {
            var url = jQuery(this).val();
            if (url) {
                window.location = 'publisher-conversion-report.php?dt=<?php echo $_GET['dt'];?>&start=<?php  echo $_GET['start']; ?>&end=<?php  echo $_GET['end']; ?>&row=' + this.value
            }
            return false;
        });
    });
